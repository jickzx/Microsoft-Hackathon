...
function detectOrganizer(text: string) {
  const match = text.match(
    /\b([A-Z][A-Za-z& ]{2,50}(?:Club|Society|Office|Center|Centre|Department|Lab|Association|Union|Council))\b/
  );
...
function detectApplyUrl(text: string, url?: string) {
  const inlineUrl = text.match(/\bhttps?:\/\/[^\s)]+/i)?.[0];
...
export function extractLocally(input: ExtractQuestRequest): QuestCard[] {
  ...
  const contactEmail = detectContactEmail(content);
  const applyUrl = detectApplyUrl(content, input.url);
  const organizer = detectOrganizer(content) ?? "Campus submitter";
  ...
  return [
    finalizeCard({
      ...
      aiExtraction: {
        confidence: 0.67,
        missingFields: organizer === "Campus submitter" ? ["organizer"] : [],
        extractedAt: now,
